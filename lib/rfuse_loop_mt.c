...
static void *rfuse_do_work(void *data)
{
	...
	while (!fuse_session_exited(mt->se)) {
		...
		**/
		...
		pthread_mutex_unlock(&mt->lock);
		//printf("aaaRFUSE: worker %p (riq_id=%d) is about to read queue (forget=%d)\n", 
        //        (void *)w, mt->riq_id, isforget);
		// printf("aaaRFUSE: mt->numavail=%d, mt->numworker=%d\n", 
		// 	   mt->numavail, mt->numworker);
		//printf("aaaRFUSE: session exited? %d\n", fuse_session_exited(mt->se));
		
		processed = rfuse_read_queue(w, mt, NULL, isforget);
		// printf("aaaRFUSE: worker %p processed=%d (forget=%d)\n", 
		// 	   (void *)w, processed, isforget);
		pthread_mutex_lock(&mt->lock);
		if(!isforget)
			mt->numavail++;
		isforget++;
		if (isforget == 2)
			isforget = 0;
		//printf("aaaRFUSEreadqueuelater(riq_id=%d): mt->numavail=%d, mt->numworker=%d,mt->max_idle=%d(forget=%d)\n", 
			//   mt->riq_id,mt->numavail, mt->numworker,mt->max_idle,isforget);
		if (mt->numavail > mt->max_idle) {
			...
			//printf("rfuse: Userworkersleep%d\n",mt->riq_id);
			pthread_mutex_unlock(&mt->lock);
			struct ioctl_args {
				int riq_id;
				int req_index;
...
int rfuse_loop_start_thread(struct rfuse_mt *mt)
{
	...
	//printf("aaaRFUSE: starting new worker thread for riq_id=%d, worker=%p\n", mt->riq_id, w);
...
void *rfuse_session_loop_mt_mriq(void *data)
{
	...
	mt.numworker = 0;
	mt.numavail = 0;
	mt.max_idle = 16;//aaaconfig->max_idle_threads
