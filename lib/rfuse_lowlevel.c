							...
							const struct fuse_file_info *f)
{
	...
	if (f->direct_io)
		arg->open_flags |= FOPEN_DIRECT_IO;//ddd
	if (f->keep_cache)
...
static void rfuse_do_lookup(fuse_req_t u_req, fuse_ino_t nodeid)
{
	...
	//printf("aaarfuse_do_lookup111\n");
	if (u_req->se->op.lookup){
		//printf("aaarfuse_do_lookup222\n");
...
static __attribute__((no_sanitize("thread"))) void rfuse_do_init(fuse_req_t u_req, fuse_ino_t nodeid)
{
	struct rfuse_iqueue *riq = u_req->riq;
	struct rfuse_req *r_req = &riq->ureq[u_req->index];
	...
	if (arg->minor >= 6)
	{
		...
		if (arg->flags & FUSE_INIT_EXT)
			arg->flags = arg->flags | (uint64_t)arg->flags2 << 32;
	...
	{
#ifdef HAVE_SPLICE
		se->conn.capable_ext |= FUSE_CAP_SPLICE_WRITE | FUSE_CAP_SPLICE_MOVE;
...
//&& (se->conn.capable_ext & (cap))) \se->conn.want_ext |= (cap){
	...
	LL_SET_DEFAULT(1, FUSE_CAP_ATOMIC_O_TRUNC);//dddddd
	//LL_SET_DEFAULT(1,FUSE_CAP_WRITEBACK_CACHE);//dddddd
	...
	if (se->conn.want_ext & FUSE_CAP_HANDLE_KILLPRIV)
		outarg->flags |= FUSE_HANDLE_KILLPRIV;
	...
	if (se->conn.want_ext & FUSE_CAP_DIRECT_IO_ALLOW_MMAP)
		outarg->flags |= FUSE_DIRECT_IO_ALLOW_MMAP;
	...
	if (se->conn.want_ext & FUSE_CAP_NO_EXPORT_SUPPORT)
		outarg->flags |= FUSE_NO_EXPORT_SUPPORT;
...
bool rfuse_read_queue(struct rfuse_worker *w, struct rfuse_mt *mt, struct fuse_chan *ch, int forget)
{
	...
	//printf("aaaRFUSE Pending queue start");
	...
	{	
		...
		if (!forget_entry)
		{
			//printf("aaa noRFUSE forget queuetarget_entry\n");
			pthread_mutex_unlock(&se->riq_lock[riq_id]);
			// rfuse_free_req(u_req);
			goto out;
		}
		//printf("aaaforgetqueue\n");
	...
	{
		// 2. Pending Requests
		//printf("aaaRFUSE Pending queue start riq %d\n",riq_id);
		pthread_mutex_lock(&se->riq_lock[riq_id]);
		target_entry = rfuse_read_pending_head(riq);
		if (!target_entry)
		{
			//printf("aaa noRFUSE Pending queuetarget_entry\n");
			pthread_mutex_unlock(&se->riq_lock[riq_id]);
			// rfuse_free_req(u_req);
			goto out;
		}
		...
		r_req = &riq->ureq[u_req->index];
		//printf("aaaRFUSE Pending queue %d\n",r_req->in.opcode);
		//printf("aaar_req->riq_id %d:::%d\n",r_req->riq_id,u_req->riq_id);
